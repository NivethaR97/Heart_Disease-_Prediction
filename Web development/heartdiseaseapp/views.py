...
class dataUploadView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            data_sex=request.POST.get('sex')
            data_cp=request.POST.get('cp')
            data_chol=request.POST.get('chol')
            data_thalach=request.POST.get('thalach')
            data_exang=request.POST.get('exang')
            data_oldpeak=request.POST.get('oldpeak')
            data_ca=request.POST.get('ca')
            #print (data)
            ...
            data = np.array([data_age,data_sex,data_cp,data_chol,data_thalach,data_exang,data_oldpeak,data_ca])
            #sc = StandardScaler()
...
#data_for_prediction =obj.pca(np.array(data_for_prediction),y_test)
            ...
            return render(request, "succ_msg.html", {'data_age':data_age,'data_sex':data_sex,'data_cp':data_cp,'data_chol':data_chol,'data_thalach':data_thalach,'data_exang':data_exang,'data_oldpeak':data_oldpeak,'data_ca':data_ca,
                                                        'out':out})
